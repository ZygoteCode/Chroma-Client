...
public class Block
{
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity)
    {
        ...
        EventBlockBounds event = new EventBlockBounds(pos, axisalignedbb);
        EventManager.call(event);
        
        axisalignedbb = event.getBox();
        pos = event.getPos();

        if (!event.isCancelled() && axisalignedbb != null && mask.intersectsWith(axisalignedbb))
        {
    ...
    public float getPlayerRelativeBlockHardness(EntityPlayer playerIn, World worldIn, BlockPos pos)
    {
        ...
        return f < 0.0F ? 0.0F : (!playerIn.canHarvestBlock(this) ? playerIn.getToolDigEfficiency(this) / f / 100.0F : playerIn.getToolDigEfficiency(this) / f / 30.0F) * getHardnessModifier();
    }
    
    public float getHardnessModifier()
    {
        if (!Minecraft.getMinecraft().getChroma().getModuleManager().getModuleByID(69).isToggled())
        {
            return 1.0f;
        }
        
        if (Minecraft.getMinecraft().getChroma().getSetManager().getSettingById(207).getValString() != "Normal")
        {
            return 1.0f;
        }
        
        return (float) Minecraft.getMinecraft().getChroma().getSetManager().getSettingById(208).getValueD();
    }
    ...
    {
    	EventGetAmbientOcclusionLightValue event = new EventGetAmbientOcclusionLightValue(this.isBlockNormalCube() ? 0.2F : 1.0F, this.getDefaultState());
