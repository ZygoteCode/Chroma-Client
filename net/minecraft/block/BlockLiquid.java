...
import net.minecraft.client.particle.chroma.event.events.EventLiquidBoundingBox;
...
public abstract class BlockLiquid extends Block
{
    ...
    {
        return hitIfLiquid && ((Integer)state.getValue(LEVEL)).intValue() == 0 || Minecraft.getMinecraft().getChroma().getModuleManager().getModuleByID(124).isToggled();
    }
    ...
    {
    	EventLiquidBoundingBox event = new EventLiquidBoundingBox(worldIn, pos, state);
    	EventManager.call(event);
    	
    	if (event.isCancelled())
    	{
    		return new AxisAlignedBB((double)pos.getX() + this.minX, (double)pos.getY() - 1.0, (double)pos.getZ() + this.minZ, (double)pos.getX() + this.maxX, event.getPos().getY(), (double)pos.getZ() + this.maxZ);
    	}
