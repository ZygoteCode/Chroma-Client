...
import net.minecraft.client.particle.chroma.event.events.EventBlockMove;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    {
    	EventBlockMove event = new EventBlockMove();
    	EventManager.call(event);
        return this.getHealth() <= 0.0F && !event.isCancelled();
    }
    ...
    protected void jump()
    {
    	EventJump event = new EventJump(0D, 0D, 0F, 0D, 0D);
    	event.setMotionUpward((double) this.getJumpUpwardsMotion());
    	
    	if (this.isPotionActive(Potion.jump))
    	{
    		event.setMotionPotion((double)((float)(this.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1F));
    	}
    	
    	if (this.isSprinting())
    	{
    		event.setF(this.rotationYaw * 0.017453292F);
    		event.setMotionX((double)(MathHelper.sin(this.rotationYaw * 0.017453292F) * 0.2F));
    		event.setMotionZ((double)(MathHelper.cos(this.rotationYaw * 0.017453292F) * 0.2F));
    	}
        ...
        this.motionY = event.getMotionUpward();

        if (this.isPotionActive(Potion.jump))
        {
            this.motionY += event.getMotionPotion();
        }
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.isJumping)
        {
            ...
            {
                this.jump();
                this.jumpTicks = Minecraft.getMinecraft().getChroma().getModuleManager().getModuleByID(120).isToggled() ? 0 : 10;
            }
        }
        else
        {
