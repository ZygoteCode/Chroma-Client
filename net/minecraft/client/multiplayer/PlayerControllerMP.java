...
import net.minecraft.client.particle.chroma.event.events.EventDamageBlock;
import net.minecraft.client.particle.chroma.event.events.EventPlaceBlock;
import net.minecraft.client.particle.chroma.event.events.EventUseItem;
...
public class PlayerControllerMP
{
    ...
    {
    	EventClickBlock event = new EventClickBlock(loc, face);
        ...
        else
        {
            if (this.currentGameType.isCreative())
            {
                ...
                this.blockHitDelay = this.mc.getChroma().getModuleManager().getModuleByID(69).isToggled() ? 0 : 5;
            }
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing)
    {
    	...
    	posBlock = event.getPos();
    	directionFacing = event.getFacing();
        ...
        else if (this.isHittingPosition(posBlock))
        {
            ...
            else
            {
                ...
                else if (mc.getChroma().getModuleManager().getModuleByID(69).isToggled() && mc.getChroma().getModuleManager().getModuleByID(69).getMode().equals("Instant"))
                {
                	this.netClientHandler.addToSendQueue(new C07PacketPlayerDigging(C07PacketPlayerDigging.Action.STOP_DESTROY_BLOCK, posBlock, directionFacing));
                }
    ...
    {
    	EventBlockReach event = new EventBlockReach(this.currentGameType.isCreative() ? 5.0F : 4.5F);
    	EventManager.call(event);
    ...
    public boolean onPlayerRightClick(EntityPlayerSP player, WorldClient worldIn, ItemStack heldStack, BlockPos hitPos, EnumFacing side, Vec3 hitVec)
    {
    	...
    	EventPlaceBlock event = new EventPlaceBlock(hitPos, side, hitVec, f, f1, f2);
    	EventManager.call(event);
    ...
    {
    	EventUseItem event = new EventUseItem(itemStackIn);
    	EventManager.call(event);
    	
    	itemStackIn = event.getItemStack();
    ...
    public boolean processRightClick(EntityPlayer playerIn, World worldIn, ItemStack itemStackIn) {
        if (this.currentGameType == WorldSettings.GameType.SPECTATOR) {
            return false;
        }
        this.syncCurrentPlayItem();
    ...
    {
    	EventAttack event = new EventAttack(EventState.PRE, playerIn, targetEntity, 0, null, null);
    	EventManager.call(event);
    	...
    	EventAttack event1 = new EventAttack(EventState.POST, playerIn, targetEntity, 0, null, null);
    ...
    {
    	EventAttack event = new EventAttack(EventState.PRE, playerIn, targetEntity, 1, null, null);
    	EventManager.call(event);
    	...
    	EventAttack event1 = new EventAttack(EventState.POST, playerIn, targetEntity, 1, null, null);
    	EventManager.call(event1);
        return this.currentGameType != WorldSettings.GameType.SPECTATOR && playerIn.interactWith(targetEntity) && !event1.isCancelled();
    }
    ...
    public boolean func_178894_a(EntityPlayer p_178894_1_, Entity p_178894_2_, MovingObjectPosition p_178894_3_)
    {
    	...
    	EventAttack event = new EventAttack(EventState.PRE, p_178894_1_, p_178894_2_, 2, p_178894_3_, null);
    	EventManager.call(event);
    	...
    	p_178894_1_ = event.getPlayer();
    	p_178894_2_ = event.getEntity();
    	p_178894_3_ = event.getMoving();
    	
        this.syncCurrentPlayItem();
    	EventAttack event1 = new EventAttack(EventState.POST, p_178894_1_, p_178894_2_, 2, p_178894_3_, vec3);
    	EventManager.call(event1);
    	p_178894_1_ = event1.getPlayer();
    	p_178894_2_ = event1.getEntity();
    	p_178894_3_ = event1.getMoving();	 
        this.netClientHandler.addToSendQueue(new C02PacketUseEntity(p_178894_2_, vec3));
        return this.currentGameType != WorldSettings.GameType.SPECTATOR && p_178894_2_.interactAt(p_178894_1_, vec3) && !event1.isCancelled();
    }
