...
public abstract class Render<T extends Entity>
{
    ...
    protected void renderLivingLabel(T entityIn, String str, double x, double y, double z, int maxDistance)
    {
        ...
        if (d0 <= (double)(maxDistance * maxDistance))
        {
            ...
            float f = 1.6F;
            boolean nameTagsActive = Minecraft.getMinecraft().getChroma().getModuleManager().getModuleByID(111).isToggled();
            float f1 = 0.016666668F * f * (entityIn.getDistanceToEntity(this.renderManager.livingPlayer) > 10.0f && nameTagsActive ? entityIn.getDistanceToEntity(this.renderManager.livingPlayer) / 10.0f : 1.0f);
            GlStateManager.pushMatrix();
            GlStateManager.translate((float)x + 0.0F, (float)y + entityIn.height + 0.5F, (float)z);
            GL11.glNormal3f(0.0F, 1.0F, 0.0F);
            GlStateManager.rotate(-this.renderManager.playerViewY, 0.0F, 1.0F, 0.0F);
            GlStateManager.rotate(this.renderManager.playerViewX, 1.0F, 0.0F, 0.0F);
            GlStateManager.scale(-(f1 *= nameTagsActive ? (entityIn.boundingBox.getAverageEdgeLength() > -50.0 ? 1.0f : entityIn.getDistanceToEntity(this.renderManager.livingPlayer) / 300.0f) : 1.0f), -f1, f1);
            GlStateManager.disableLighting();
            GlStateManager.depthMask(false);
            ...
            if (entityIn instanceof EntityLivingBase && Minecraft.getMinecraft().getChroma().getModuleManager().getModuleByID(110).isToggled())
            {
                ...
                str = String.valueOf(str) + health + " §4§l" + "❤";
            }
