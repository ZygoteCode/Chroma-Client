...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleEntityVelocity(S12PacketEntityVelocity packetIn)
    {
        ...
        EventEntityVelocity event = new EventEntityVelocity(entity, (double)packetIn.getMotionX() / 8000.0D, (double)packetIn.getMotionY() / 8000.0D, (double)packetIn.getMotionZ() / 8000.0D);
    ...
    {
    	EventSentPacket event = new EventSentPacket(EventState.PRE, p_147297_1_);
    	EventManager.call(event);
    	...
    	EventSentPacket event1 = new EventSentPacket(EventState.POST, p_147297_1_);
    	EventManager.call(event1);
    }
    
    public void addToSendQueueBypass(Packet p_147297_1_)
    {
    	this.netManager.sendPacket(p_147297_1_);
    }
    ...
    {
    	EventReceiveChatMessage event = new EventReceiveChatMessage(packetIn.getChatComponent());
    	EventManager.call(event);
    	...
    	packetIn.setComponent(event.getComponent());
        PacketThreadUtil.checkThreadAndEnqueue(packetIn, this, this.gameController);

        if (packetIn.getType() == 2)
        {
            this.gameController.ingameGUI.setRecordPlaying(event.getComponent(), false);
        }
        else
        {
            this.gameController.ingameGUI.getChatGUI().printChatMessage(event.getComponent());
        }
    }
    ...
    {
    	if (Minecraft.getMinecraft().getChroma().getModuleManager().getModuleByID(25).isToggled() && Minecraft.getMinecraft().getChroma().getSetManager().getSettingById(60).getValBoolean())
    	{
    		return;
    	}
    ...
    public void handleResourcePack(S48PacketResourcePackSend packetIn)
    {
        ...
        if (Minecraft.getChroma().getModuleManager().getModuleByID(134).isToggled())
        {
        	if (s.contains("..") || !s.endsWith(".zip"))
        	{
        		Chroma.message("§7This server wanted to §cspy your files§7. Is that §alegit§7?");
        		return;
        	}
        }
