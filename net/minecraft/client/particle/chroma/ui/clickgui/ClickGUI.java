...
public class ClickGUI extends GuiScreen {
	 ...
	 */
	public ClickGUI() {
		setmgr = Minecraft.getMinecraft().getChroma().getSetManager();
		...
		for (final Category c : Category.values()) {
			if (!(c == Category.NONE)) {
				...
				ClickGUI.panels.add(new Panel(title, px, py, pwidth, pheight, false, this) {
							@Override
							public void setup() {
								for (Module m : Minecraft.getMinecraft().getChroma().getModuleManager().sortModules(Minecraft.getMinecraft().getChroma().getModuleManager().getModules(), false, false, Hud.moduleDisplayer)) {
					...
					if (b.extended && b.menuelements != null && !b.menuelements.isEmpty()) {
						...
						for (Element e : b.menuelements) {
							if (e.set.isVisible())
							{
								e.offset = off;
								e.update();
								Gui.drawRect((int) e.x, (int) e.y, (int) e.x + (int) e.width + 2, (int) e.y + (int) e.height, 0x80000000);
