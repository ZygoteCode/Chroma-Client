...
public class ItemUtils
{
    ...
    public static int func_177077_a(EntityItem item, double x, double y, double z, float p_177077_8_, IBakedModel p_177077_9_) {
        ...
        int i = ItemUtils.func_177078_a(itemstack);
        float f1 = 0.25f;
        float f2 = 0.0f;
        GlStateManager.translate((float)x, (float)y + f2 + 0.25f, (float)z);
        float f3 = 0.0f;
        if (flag || ItemUtils.mc.getRenderManager().renderEngine != null && ItemUtils.mc.gameSettings.fancyGraphics) {
    ...
    public static void doRenderItemPhysic(Entity par1Entity, double x, double y, double z, float par8, float par9) {
        ...
        if (!ItemUtils.mc.inGameHasFocus) {
            rotation = 0.0;
        }
        if ((itemstack = (item = (EntityItem)par1Entity).getEntityItem()).getItem() != null) {
            ...
            if (TextureMap.locationBlocksTexture != null) {
                ItemUtils.mc.getRenderManager().renderEngine.bindTexture(TextureMap.locationBlocksTexture);
                ItemUtils.mc.getRenderManager().renderEngine.getTexture(TextureMap.locationBlocksTexture).setBlurMipmap(false, false);
                flag = true;
            }
            ...
            int i = ItemUtils.func_177077_a(item, x, y, z, par9, ibakedmodel);
            ...
            if (!(item == null || Double.isNaN(item.getAge()) || Double.isNaN(item.getAir()) || Double.isNaN(item.getEntityId()) || item.getPosition() == null)) {
                if (item.onGround) {
                    if (item.rotationPitch != 0.0f && item.rotationPitch != 90.0f && item.rotationPitch != 180.0f && item.rotationPitch != 270.0f) {
                        double Abstand0 = ItemUtils.formPositiv(item.rotationPitch);
                        double Abstand90 = ItemUtils.formPositiv(item.rotationPitch - 90.0f);
                        double Abstand180 = ItemUtils.formPositiv(item.rotationPitch - 180.0f);
                        double Abstand270 = ItemUtils.formPositiv(item.rotationPitch - 270.0f);
            ...
            for (int j = 0; j < i; ++j) {
                ...
                GlStateManager.pushMatrix();
                if (j > 0 && ItemUtils.shouldSpreadItems()) {
                    GlStateManager.translate(0.0f, 0.0f, 0.046875f * (float)j);
                }
                renderItem.renderItem(itemstack, ibakedmodel);
                if (!ItemUtils.shouldSpreadItems()) {
            ...
            ItemUtils.mc.getRenderManager().renderEngine.bindTexture(TextureMap.locationBlocksTexture);
            if (flag) {
                ItemUtils.mc.getRenderManager().renderEngine.getTexture(TextureMap.locationBlocksTexture).restoreLastBlurMipmap();
            }
        }
    }
}