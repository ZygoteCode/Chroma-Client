...
public final class RaycastUtils
{
    ...
    {
        return raycastEntity(range, RotationUtils.getServerYaw(), RotationUtils.getServerPitch());
    }

    public static Entity raycastEntity(double range, float yaw, final float pitch)
    {
        ...
        if(renderViewEntity != null && Minecraft.getMinecraft().theWorld != null) {
            ...
            final List<Entity> entityList = Minecraft.getMinecraft().theWorld.getEntitiesInAABBexcluding(renderViewEntity, renderViewEntity.getEntityBoundingBox().addCoord(entityLook.xCoord * blockReachDistance, entityLook.yCoord * blockReachDistance, entityLook.zCoord * blockReachDistance).expand(1D, 1D, 1D), Predicates.and(EntitySelectors.NOT_SPECTATING, Entity :: canBeCollidedWith));
            ...
            for(final Entity entity : entityList) {
                ...
                if(axisAlignedBB.isVecInside(eyePosition)) {
                    ...
                    if(eyeDistance < blockReachDistance || blockReachDistance == 0.0D) {
                        if(entity == renderViewEntity.ridingEntity && !renderViewEntity.riddenByEntity.isRiding()) {
                            if(blockReachDistance == 0.0D)
                                pointedEntity = entity;
                        }else{
                            pointedEntity = entity;
